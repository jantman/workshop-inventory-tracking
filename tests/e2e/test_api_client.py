...
class TestApiClientCreateItem:
    def test_create_single_item(self, client, live_server):
        ...
        assert result.created_ja_ids == ['JA000301']

        # Verify the item is reachable through the existing read API.
        verify = requests.get(f'{live_server.url}/api/items/JA000301', timeout=10)
        assert verify.status_code == 200
        body = verify.json()
        assert body.get('success') is True
        assert body['item']['ja_id'] == 'JA000301'
...
class TestApiClientUploadPhoto:
    def test_upload_photo_for_existing_item(self, client, live_server, sample_image_path):
        ...
        upload_result = client.upload_photo('JA000350', file_path=sample_image_path)
        ...
        photos_resp = requests.get(f'{live_server.url}/api/items/JA000350/photos', timeout=10)

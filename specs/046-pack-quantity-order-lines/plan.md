...
| **IV. Test Discipline** | **Passes.** Unit tests for the title parse, the pack arithmetic, override detection and the three `line_fields`; E2E for the two pages. Every E2E wait is on an observable state — the derived quantity's value — never a duration. Template and JS changes to `app/templates/**` and `app/static/js/**` trigger the screenshot job. It is a **reminder, not a gate** -- `.github/workflows/screenshots.yml` stopped diffing in CI because CI rasterizes fonts differently and every file differed on every run (issue #77), so the constitution's "CI blocks merge on stale screenshots" is out of date. The judgment is the author's: regenerate, then commit only what the change actually altered. |

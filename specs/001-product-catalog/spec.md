...
**Input**: User description: "`docs/spec-product-catalog.md`" — Enhancement to an existing workshop inventory application, adding the ability to catalogue and identify purchased parts, supplies, and components.
...
- **FR-016**: The system MUST interpret the standard machine-readable labels used by industrial electronics distributors, extracting at least the manufacturer part number, quantity, and order references. Field boundaries within a scanned distributor label MUST be determined from the separators the scanner passes through, not guessed from the content.
...
- **FR-037**: The system MUST offer, for product labels, the same set of label stocks the application already offers for its existing inventory labels, with no stock reserved or excluded and no new stock defined.
...
- **Explicit product-equivalence records are deferred.** The edge case of one real-world product sold under several vendor listings is covered by attaching multiple identifiers and multiple purchases to a single product. A dedicated "these two products are the same thing" relationship is not built now, because no observed need requires it beyond what multiple identifiers already provide. This is a deliberate application of the constitution's Simplicity First principle; the design should not preclude adding it later.
...
- The application already defines a fixed set of label stocks for its existing inventory labels. Product labels select from that same set in full — every existing stock is available for product use, none is reserved, and no new stock is defined by this feature.
- The deployed scanner reads the 2D machine-readable symbol on industrial distributor part labels and passes the label's field separators through in a recoverable form. Distributor-label interpretation therefore relies on those separators rather than on inferring field boundaries from the content. A future scanner that does not preserve them would degrade User Story 4 to the raw-scan fallback already required by FR-016's non-conforming case.

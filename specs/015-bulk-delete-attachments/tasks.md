...
- [x] T018 Regenerate documentation screenshots — `venv/bin/nox -s screenshots_headless`, then `venv/bin/nox -s screenshots_verify` — and commit **only what this change actually moved**. Required by the constitution for any change to `app/templates/**` or `app/static/js/**`. *(Corrected while doing it: CI does **not** block on stale screenshots — that job was removed because per-machine font rendering made every glyph differ, and it now only posts a reminder. And the product detail page **is** in the set — it comes from `tests/e2e/test_screenshot_generation.py`, not from the named entries in `screenshot_config.yaml` — so US1/US2 do move `user-manual/product_detail.png`. In the event, regeneration rewrote 12 PNGs and 5 were this feature; the other 7 differed by ~0.1% in regions holding generated dates and were reverted. See [research.md](./research.md) §9.)*

...
- [X] T030 [P] [US4] Replace `catalogue` → `catalog` throughout `docs/spec-product-catalog.md` (22 occurrences, all prose).
- [X] T031 [P] [US4] Replace `catalogue` → `catalog` throughout `docs/product-functionality-gap.md` (5 occurrences, all prose).
- [X] T032 [P] [US4] Replace the remaining `catalogue` occurrences in `docs/user-manual.md` body prose. Most headings were already rewritten in T007; this catches the sentences.
- [X] T033 [P] [US4] Fix the two user-visible strings: `app/templates/product/reorder.html:15` ("worked out from what the catalogue already knows") and `app/templates/product/detail.html:29` ("This scan matched a product already in the catalogue"). Confirmed at planning time that no test asserts on either. Satisfies FR-009.
- [X] T034 [P] [US4] Replace `catalogue` → `catalog` in comments and docstrings under `app/` — 29 Python comments/docstrings plus 4 Jinja comments in `_layout.html`, `_rename_modal.html` and `detail.html`. Satisfies FR-010 for `app/`.
- [X] T035 [US4] Replace `catalogue` → `catalog` in comments and docstrings under `tests/`. Do this **before** the identifier renames below so the two passes do not collide in the same files.
- [X] T036 [P] [US4] Rename the pytest fixture `catalogue` → `catalog` in `tests/unit/test_product_search.py` (definition at line 22, 71 references in the file). A missed reference fails immediately with `fixture 'catalogue' not found`.
- [X] T037 [P] [US4] Rename the pytest fixture `catalogued` → `cataloged` in `tests/unit/test_capture.py` (definition at line 435, 20 references in the file).

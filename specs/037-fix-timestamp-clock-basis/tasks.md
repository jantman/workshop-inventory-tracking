...
- [X] T016 [US2] Add to `tests/unit/test_clock_basis.py`: under `forced_timezone`, seed `quantity_updated_at = utc_now() - timedelta(hours=3)` directly through a session (the `backdate` helper at `tests/unit/test_stock_status.py:28` is the established pattern and its docstring explains why the service cannot produce a backdated age), then assert `product.quantity_age` is within a minute of three hours (FR-007, INV-4). Add the mirror assertion for `stock_status_age`
- [X] T017 [US2] Run the unit suite and confirm T016 **fails after Phase 3** — it should report an age near seven hours, or a negative one, because the write side has moved and the read side has not. A pass here means Phase 3 is incomplete
...
- [X] T020 [P] [US2] In `tests/e2e/test_reorder_view.py`, retarget the two `datetime.now() - timedelta(...)` seeds at :236 and :239 onto `utc_now()`. Same reasoning as T019; the ages are 9 and 800 days
- [X] T021 [US2] Run `PATH="$HOME/.pyenv/versions/3.13.12/bin:$PATH" venv/bin/nox -s tests` and confirm green, T016 included
...
- [X] T029 Add a short **Timestamps** note to `_bmad-output/project-context.md` recording the convention: recorded instants come from `app.utils.clock.utc_now()` and are naive UTC; days the operator states come from `local_now()`; a bare `datetime.now()` in a service is a bug outside the logging and report sites. This is the only thing that keeps the next feature from reintroducing the defect
- [X] T030 Re-run quickstart.md §1 against the finished code and confirm the gap is under a second, with `TZ=America/New_York` forced. It was 3:59:59 before
- [X] T031 Run `PATH="$HOME/.pyenv/versions/3.13.12/bin:$PATH" venv/bin/nox -s e2e` **detached** (`nohup` + poll — a foreground run reports a false timeout at 10 minutes on a suite that takes about 13m 45s). Two groups matter more than the rest: the **active-status and history** tests, because `inventory_items.date_added` selects the current history row and this feature is under Constitution VI whether or not it looks like it (research.md R8); and `tests/e2e/test_stock_age.py` plus `tests/e2e/test_reorder_view.py`, whose seeds T019 and T020 retargeted
- [X] T032 Run `git status --short` after the e2e session and confirm the working tree is clean. Anything under `docs/images/screenshots/` means screenshot tests were selected and the run is wrong (Constitution IV)
- [ ] T033 (not done -- no deployed instance is available to this session. The automated
      coverage is: the four-timestamps check is `TestARecordedRowIsOnOneClock` in
      `tests/unit/test_clock_basis.py`; the age rendering is
      `tests/e2e/test_stock_age.py` and `test_reorder_view.py`, both green; the evening
      order date is `TestStatedDaysStayOnTheOperatorsCalendar`, which patches the clock
      rather than waiting for 20:00. What is left uncovered is the human eyeball on a
      real MariaDB, and the one-time age jump described in quickstart.md, which by
      definition only shows on a database that has pre-fix rows in it)
      Walk quickstart.md §3 by hand against a running instance: the four `products` timestamps within seconds of each other on a product created after the fix; a product page whose ages read in hours rather than "just now"; and — **only observable after 20:00 local** — an order captured with no date typed showing today's day on the order listing, not tomorrow's

...
- [ ] T004 [P] Add `DIGIKEY_CLIENT_ID`, `DIGIKEY_CLIENT_SECRET`, `DIGIKEY_ACCOUNT_ID` and `DIGIKEY_API_BASE` (default `https://api.digikey.com`) to `config.py`, in both `Config` and `TestConfig`, following the shape of the adjacent `GOOGLE_*` settings. All four are already set in the developer's `.env`
- [ ] T005 [P] Document the four settings in `.env.example` with the registration steps as comments. Confirm `.env` stays gitignored

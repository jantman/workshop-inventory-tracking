...
- **Order data is account-scoped and part data is not.** Reading back one's own orders requires an account-level authorization that reading public part data does not. FR-035 assumes these are separable; if DigiKey's terms turn out to bind them together, the two-tier behaviour collapses to one and FR-035 becomes vacuous rather than wrong.

...
handling for them here without a corresponding requirement.
"""
...
class InvalidGtinError(ValueError):
    """
    Raised when a value is not a valid GTIN (non-digit, wrong length, or a
    failed mod-10 check digit). A plain `ValueError` subclass so this module
    stays free of any app/framework dependency; callers translate it into a
    domain error.
    """
...
def normalize_gtin(value: str) -> str:
    ...
    in {8, 12, 13, 14}; left-zero-pad to 14; verify the GS1 mod-10 check digit
    over the padded 14 digits.
    ...
    Raises:
        InvalidGtinError: if the input is not a string, is not all ASCII
            digits, has an unsupported length, or fails check-digit validation.

...
- **Product Catalog**: Catalog what you buy -- what a part is, what it cost, and where it came from. Manufacturer part numbers, retail barcodes and vendor item ids on every product, with a generated internal code that makes it scannable before any label is printed; scan any code to open the product or start a filled-in create form; whole-order capture from Amazon, DigiKey and McMaster-Carr -- one click or one order number records every line of an order as an outstanding purchase, with one screen listing what is still on its way from anyone; purchase and order tracking, including a derived reorder list that marks what is already on the way; and categories and tags that are created by using them, with renames that carry every product beneath them

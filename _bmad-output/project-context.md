...
- **`nox -s doctests` executes the `>>>` examples in `app/utils/` only** (`pytest --doctest-modules app/utils`). Doctests are deliberately not enabled repo-wide, and the explicit path argument is what scopes them — don't move that flag into `pytest.ini`. Docstring examples you add under `app/utils/` become executed tests. A `>>>` prompt under `app/` outside `app/utils/`, or in a repo-root module, now **fails** `tests/unit/test_doctest_scope.py` (run by `nox -s tests`), which names each offending `path:line`; that test also fails if the `doctests` session stops existing or being a nox session, if its `app/utils` path argument is widened, retargeted or dropped, if the session subtracts part of `app/utils` with `--ignore`/`--deselect`, if `--doctest-modules` leaks into `pytest.ini`'s `addopts`, or if `doctests` leaves `nox.options.sessions`. Its scan reaches no further than those two places — a prompt under `tests/`, `migrations/`, `.claude/skills/**/scripts/`, `_bmad/scripts/` or `.bmad-loop/` is still simply never run, unreported. So a behavior worth pinning with an example belongs in a pure utility module under `app/utils/`, the only tree that executes one; anywhere else, state it in prose without a `>>>` prompt.
...
- **Markers gate scope — register them in `pytest.ini`, the single source of truth.** `pytest.ini` declares `[pytest]` and is fully live: its `markers` list (`unit`, `integration`, `e2e`, `slow`, `screenshot`), `addopts`, `testpaths = tests` and `norecursedirs` all apply. `--strict-markers` **is** in force, so an unregistered marker is a collection error — add new markers to the ini's `markers` list. `tests/conftest.py` registers none, and it isn't loaded at all by the `doctests` session, which is why the ini (not a conftest) is the right home. `tests/unit/test_pytest_config.py` fails loudly if the ini ever stops being read.
- **Consequences of the live `addopts` (`--verbose --tb=short --strict-markers --strict-config --disable-warnings`):** `--disable-warnings` suppresses the warnings *summary* in every session, so a run reports a bare count. To see the detail, re-run with `--override-ini="addopts=--verbose --tb=short --strict-markers --strict-config"` — `--override-ini` **replaces** the whole option string rather than editing it, so you must repeat the flags you want to keep; dropping the two strict flags fails `tests/unit/test_pytest_config.py` by design. `--strict-config` makes a misspelled key in `pytest.ini` an error.
- **How `addopts` combines with the per-session flags in `noxfile.py`:** pytest prepends `addopts` to the command line, so session flags are *appended* to it, not substituted for it. What that means per option type: **counting** options accumulate (each session's `-v` stacks with the ini's `--verbose`, giving `-vv`); **store** options take the last value, so a session's `--tb=long` or `-m ...` wins over the ini's rather than merging with it; **path arguments** replace `testpaths` entirely. So putting a store-type flag in `addopts` sets a default a session can override — it does not compose.
...
- **Layout:** `tests/unit/`, `tests/integration/` and `tests/e2e/`; files `test_*.py`/`*_test.py`, classes `Test*`, functions `test_*` (set in `pytest.ini`). `migrations/`, `venv/`, `.nox/` and other dotted dirs are excluded from collection via `norecursedirs` — which filters *recursion* only, so naming such a path directly (`pytest .nox/...`) still collects it.

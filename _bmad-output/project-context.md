...
- **McMaster-Carr capture** (feature 028): **no API and no configuration** — their API needs an application review a one-person workshop will not pass. Orders and products are read off the page in the operator's browser by `extension/capture-agent.js`, injected by the browser extension, which dispatches on the **URL path** (never the hostname, or the e2e harness could not drive it). There is nothing to set up and no credential to hold.
...
- **Versioning:** SemVer. `version` in `pyproject.toml` is the single source of truth; `app/version.py` reads it at runtime. `pyproject.toml` is metadata only — the app is never pip-installed as a package. What the footer and `/health` report is that number plus a build suffix: `app/version.py` exposes the bare number as `RELEASE_VERSION` and the reported string as `__version__` (`0.1.1`, `0.1.1-6d15bde`, `0.1.1-6d15bde-dirty`). Images are stamped via the `BUILD_SHA` build arg by `test.yml` and deliberately not by `release.yml`; a working copy is asked with `git`. See `docs/deployment-guide.md`.
...
- **`ScanResolution.outcome` has four members**, not three: `product`, `receive`, `create`, `search`. In the ECIA branch of `resolve_scan`, the order-line lookup **must run before** the `1P` → MPN lookup, or a bag for a part you already own opens the product page instead of its receipt. **The same rule holds in the FREE_TEXT branch**: `find_mcmaster_receivable` runs before the vendor-scoped identifier lookup, for exactly the same reason — capturing an order creates products carrying those part numbers.
- **McMaster shows no order number.** Only the customer's *Purchase Order* string, which is editable on their site and auto-generates as MMDD+SURNAME. It is what `supplier_order_reference` holds and what the order screen is keyed by; `purchases.vendor_order_id` holds the stable id from the order's URL, and re-capture pairs on that first so a rename does not duplicate every line.
- **Two McMaster pages, two markup conventions.** The order card view uses plain unhashed class names (`dtl-row`); product pages are CSS-module hashed (`_price_1y02s_5`), so product selectors must match the readable stem with `[class*="..."]` and never a whole class name.
- **A captured McMaster order is derived, not stored**, exactly as a DigiKey one is. Do not add a table for it.

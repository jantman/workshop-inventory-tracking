...
status: done 2026-07-28
resolution: already resolved: The entry's own second closure option was taken and is now stated in code: app/mariadb_catalog_service.py `resolve_scan` docstring -- "Read-only, like search_products: there is no commit, no rollback and no audit log, so scan resolution is idempotent by construction" -- and app/main/routes.py `api_scan` docstring: "It stays READ-ONLY, deliberately. resolve_scan writes nothing ... which is what keeps a rescan after the client's 10s timeout free rather than a double-apply." The premise ("once /api/scan has side effects") never materialised; resolution landed and the endpoint was kept effect-free.
...
status: open
decision: 2026-07-28 Coerce with str() then bound — Coerce every text-column value with `str()` at the boundary and then apply the existing length rule to the coerced string, so `{"vendor": 5}` keeps storing `'5'` and `{"vendor": true}` keeps storing `'1'` while `{"vendor": ["a"*300]}` and `{"vendor": 10**400}` earn a field-named 400 instead of a 500. This preserves every currently-successful request and only converts the 500s into proper refusals.
...
status: open
decision: 2026-07-28 Tighten the price on all three entry points — Give `_purchase_unit_price` an explicit spelling rule - ASCII digits, one optional leading sign, at most one decimal point - applied before `Decimal()`, so `'1_0'` and `'\u0665'` are refused with the existing field message on the HTML purchase form, the create form's First Receipt card and the JSON endpoint at once. Hold the HTML refusal message strings unchanged where tests pin them, update the docstring's record of deliberate leniency, and re-point `test_both_forms_accept_and_store_the_same_price` at a valid price. This closes DW-186 in the same motion by giving Quantity and Unit Price one definition of a number.
...
status: done 2026-07-28
resolution: resolved by sweep bundle dw-suggestion-order-collation-tiebreak
...
status: done 2026-07-28
resolution: resolved by sweep bundle dw-logging-redaction-completeness
...
status: open
decision: 2026-07-28 Bound and scrub the traceback channel only — Do not add a general value-pattern scan. Instead close the one named channel: stop embedding `traceback.format_exc()` verbatim in `error_details` at app/main/routes.py:4188,4190,4230,4232 - log the exception type and message as structured fields and route the full traceback to the standard exception channel rather than into an audit payload - and bound the caller-controlled fields (`request.url`, `user_agent`) the formatter attaches to every record. Restate the redaction docstring's boundary so the remaining gap is explicit.
...
status: open
decision: 2026-07-28 Annotate with a dated correction — Leave the original sentences intact and append a dated correction note beneath each falsified claim in all five locations (3-1:201; 3-2:33,92; 3-3:35,166), naming what changed, when, and where the current truth lives (the canonical e2e isolation note in tests/e2e/conftest.py). This preserves the artifacts as historical records while stopping them from instructing future authors wrongly. Adopt this as the project's stated convention for stale implementation artifacts.
...
status: open
decision: 2026-07-28 Drop the shortcut and its documentation — Remove the `Focus Search` entry from the shortcut table, its line from the in-app help modal (app/static/js/main.js:196), and its entry from `docs/user-manual.md:1943`, so nothing advertises a shortcut that does nothing. This also removes the ordering hazard DW-135 records, since `Shift`+`/` then matches only `Help`. Leave arming the scan field to Epic 9 Story 9.1, which owns that question.
...
status: open
decision: 2026-07-28 Classify arguments: drop non-text, halve text — Give `_bounded_scan_url` a per-argument classification instead of one type test: free-text arguments (`q`, `description`, `notes`) may be truncated, while enumerated and coupled arguments (`scan_kind`, `identifier_type`/`identifier_value` as a unit, `quantity`, `order_number`, `mpn`, `vendor_sku`) are dropped whole rather than halved into a plausible-but-wrong value. Enforce that the ECIA create arm never ends with every pre-fill dropped and no `description` (FR40) by adding a `description` fallback before the last pre-fill goes. Pin the identifier pair travelling together and a normalized GTIN never being truncated. This also settles DW-146's second option.
...
status: open
decision: 2026-07-28 State the deployment shape, then size and handle — Take the operator's worker/thread count and MariaDB `max_connections` as given inputs, set `pool_size`/`max_overflow`/`pool_timeout` from them (documented in config.py with the arithmetic that produced them and surfaced as env-overridable settings), and add a handler for `sqlalchemy.exc.TimeoutError` that answers a 503 with a retry-after rather than an anonymous 500. Add a startup log line stating the effective connection ceiling so a mismatch with the server's limit is visible at boot.
...
status: open
decision: 2026-07-28 Explicit connect_timeout plus per-app lock — Set an explicit `connect_args={'connect_timeout': ...}` in `Config.SQLALCHEMY_ENGINE_OPTIONS` so a connect attempt is bounded by a value the project chose rather than the driver's TCP default, and move the creation lock from module scope into `app.extensions` so one app's first touch cannot block another's. Keep the 'caches nothing on failure' contract and its test intact - with a bounded attempt, serialized retries stop being an outage amplifier.
...
status: open
decision: 2026-07-28 Leave it as recorded
...
status: open
decision: 2026-07-28 Partial-update, like its neighbours — Make an absent `description` key mean 'leave the description alone' on `product_edit`: gate the requiredness rule on the key being present, and key the write on `field in form_data` the way the four neighbouring fields already are, so a partial POST that omits `description` succeeds and one that submits it blank is still refused. Keep `product_add`'s unconditional requirement, where there is no stored value to leave alone, and pin both routes' behaviour.
...
status: open
decision: 2026-07-28 Refuse on both branches when the baseline is unreadable — Treat an unreadable stored baseline as a failure the operator must see: on GET, answer the project's storage-error page rather than a blank form; on POST, keep the submitted values but refuse to render a form whose blank fields would silently clear stored data - re-render with the fields the submission carried and an explicit refusal for the rest, or answer the error page. Make `_render_data`'s comment and the GET branch agree, so the route has one stated position.
...
status: open
decision: 2026-07-28 Keep the caps, document the discrepancy — Keep `maxlength` as a cheap client-side guard that is correct for the ASCII and BMP text these fields overwhelmingly carry, and record the code-unit versus code-point discrepancy explicitly - a comment beside each attribute and a line in the field-limits docstring - so the next reader does not conclude the two bounds agree. Add a test pinning that the server, not the browser, is the authority.
...
status: open
decision: 2026-07-28 Companion method returning (resolution, searched_text) — Add a companion service method - `resolve_scan_with_search_text` or equivalent - returning `(ScanResolution, searched_text)` and have `api_scan` call it instead of `resolve_scan` followed by `scan_search_text`. Leave the frozen `ScanResolution` dataclass and `resolve_scan`'s signature untouched so AD-15 holds. This removes the fifth session and closes the TOCTOU window in one change; update `test_the_endpoint_opens_no_more_sessions_than_api_scan_claims` to the new count and pin that `q` names the candidate `hit_count` was counted from.
...
status: open
decision: 2026-07-28 Autocomplete over existing scopes, stored as typed — Add `vendor_scope` to `FIELD_SUGGESTION_COLUMNS` and wire the Vendor Scope input to `field-autocomplete.js`, which the create form already loads, so the operator is steered toward a scope already in use without being constrained to one. Store the value as typed apart from `.strip()`, and add an integration test pinning the MariaDB folding behaviour so the SQLite/MariaDB divergence is asserted rather than assumed. Lowest-cost option that attacks the typo cause without inventing a vocabulary.
...
status: open
decision: 2026-07-28 Refuse at the service — Make `add_identifier` raise `ValidationError` when a `VENDOR_SKU`/`ASIN`/`FNSKU` arrives with no vendor, matching how the GTIN check twenty lines above treats an unusable identifier, so the guarantee lives where the house rule says guarantees live and the future identifier-management surface inherits it. Audit and update `tests/unit/test_catalog_service.py::TestCatalogServiceIdentifiers` - notably `test_add_each_type_persists`, which currently adds vendor-scoped types with no vendor - and confirm no other caller relies on the `''` coercion.
...
status: done 2026-07-28
resolution: closed by human decision: Accept the two populations: legacy rows keep the global namespace, which is a legal state the constraint permits, and the mixed reachability is recorded in the identifier documentation and in `add_identifier`'s docstring. Repairing individual rows waits for the identifier-management surface that DW-26 and DW-180 also wait on, where a human can see the product's purchase history and choose the vendor - which is the only place the true scope is knowable.
decision: 2026-07-28 Leave global, document, revisit with the management surface — Accept the two populations: legacy rows keep the global namespace, which is a legal state the constraint permits, and the mixed reachability is recorded in the identifier documentation and in `add_identifier`'s docstring. Repairing individual rows waits for the identifier-management surface that DW-26 and DW-180 also wait on, where a human can see the product's purchase history and choose the vendor - which is the only place the true scope is knowable.
...
status: done 2026-07-28
resolution: closed by human decision: Leave `identifier_vendor` unpopulated by the scan path. No ECIA data identifier names the distributor reliably enough to found a uniqueness namespace without the operator confirming it, and a silently pre-filled wrong scope is unrepairable today (DW-183). The operator confirms the vendor by typing it, steered by the autocomplete DW-179 recommends. Keep the whitelist entry, which exists so the field survives a failed submit.
decision: 2026-07-28 Do not pre-fill; steer with autocomplete — Leave `identifier_vendor` unpopulated by the scan path. No ECIA data identifier names the distributor reliably enough to found a uniqueness namespace without the operator confirming it, and a silently pre-filled wrong scope is unrepairable today (DW-183). The operator confirms the vendor by typing it, steered by the autocomplete DW-179 recommends. Keep the whitelist entry, which exists so the field survives a failed submit.
...
status: open
decision: 2026-07-28 Fix only the help text — Keep the decided behaviour exactly and stop the form from marketing the bypass: reword `purchase_add.html:47` so it states that a blank Order Date is recorded as today and will be compared as today, so an operator reading the control knows the blank does not exempt the pair. No route or service change.
...
status: open
decision: 2026-07-28 Accept and measure — Keep the decided trigger and close the measurement gap instead: add the `K`-bearing envelope shape to the e2e suite alongside `test_a_part_number_only_envelope_saved_records_no_purchase`, so the accepted behaviour is asserted rather than merely tolerated, and record in the ledger note what would have to be observed on real labels to reopen the question. No behaviour change.
...
status: open
decision: 2026-07-28 Plumb pre-fill origin, advise only on typed values — Carry 'what the query string pre-filled' through the create POST (a hidden field naming the pre-filled keys, compared against the submitted values) so the route can tell a scan-supplied `vendor_sku` from a typed one, and emit a non-fatal advisory through the existing `followup_errors` channel only when the dropped receipt values were typed by a human. This also gives DW-193 the mechanism its third option needs. Pin both directions: a scan-and-catalogue save is silent, a hand-typed partial receipt advises.
...
status: done 2026-07-28
resolution: closed by human decision: Accept that an all-zero GTIN row is a recorded wedge no-read, not a real trade item number, and that its unreachability by lookup is the intended consequence of DW-69 extended to data that should not have been admitted. It remains visible through the `product_identifiers.value` search arm and can be removed by hand if it exists. Record the reasoning alongside DW-69's decision.
decision: 2026-07-28 Accept as unreachable — Accept that an all-zero GTIN row is a recorded wedge no-read, not a real trade item number, and that its unreachability by lookup is the intended consequence of DW-69 extended to data that should not have been admitted. It remains visible through the `product_identifiers.value` search arm and can be removed by hand if it exists. Record the reasoning alongside DW-69's decision.
...
status: done 2026-07-28
resolution: closed by human decision: Accept that a no-read lands on a search page showing the query it searched for, which an operator can read and recognize as `00000000`, and that adding no-read logic to the service contradicts DW-69's deliberate confinement of that rule to `app/utils/gtin.py`. Closes on the reasoning that the operator sees the query; DW-202 and DW-204 close with it.
decision: 2026-07-28 Accept the misleading hits — Accept that a no-read lands on a search page showing the query it searched for, which an operator can read and recognize as `00000000`, and that adding no-read logic to the service contradicts DW-69's deliberate confinement of that rule to `app/utils/gtin.py`. Closes on the reasoning that the operator sees the query; DW-202 and DW-204 close with it.
...
status: done 2026-07-28
resolution: already resolved: Premise refuted and the in-code copies corrected. app/main/routes.py `_purchase_date` docstring now states "MariaDB's 1000-01-01 floor is the range its DATE type is DOCUMENTED to support, not one it enforces -- 11.8 under STRICT_TRANS_TABLES stores '0999-01-01' and '0001-01-01' and reads them back with no error and no warning", and tests/unit/test_product_routes.py's ('0999-01-01','stored') row carries the same correction. There is no 500 to guard and the proposed 1000<=year<=9999 check would refuse values the database stores (measured under DW-216). DW-214's one surviving clause -- neither purchase entry point has MariaDB integration coverage -- is owned by DW-216, bundled here as mariadb-integration-tier-round-2.
...
status: open
decision: 2026-07-28 Emit money as a string — Change `Purchase.to_dict` to render `unit_price` as `str(self.unit_price)` (the two-place Decimal spelling the column holds), update the api_record_purchase 201-body assertions and any consumer in app/api_client.py, and state the rule -- money crosses the AD-13 boundary as a decimal string -- next to the column comment that already says 'never float'. Take it together with DW-215's first reference doc for the endpoint so the contract is written down where a caller will find it.
...
status: open
decision: 2026-07-28 Restate as a self-imposed budget — Leave both numbers at 7000 and rewrite the rationale comments in app/main/routes.py:2641,:2729 and app/static/js/field-autocomplete.js:88-93 to say what is true: 7000 is a self-imposed URL budget chosen to sit under a common request-line ceiling, not a fact about this deployment's server. Keep the tripwire that pins the two together.
...
### DW-224: Determinism was bought by always offering the MOST-UPPERCASE spelling, and nobody weighed that against the alternatives
origin: spec-suggestion-order-collation-tiebreak-review-2
source_spec: `_bmad-output/implementation-artifacts/spec-suggestion-order-collation-tiebreak.md`
location: `app/db.py` (`binary_order_key`, `BINARY_COLLATION`), `app/mariadb_inventory_service.py::get_field_value_suggestions`, `app/mariadb_catalog_service.py::get_field_value_suggestions`
severity: medium
summary: `utf8mb4_bin` orders uppercase ASCII before lowercase, so where a value exists in several casings the operator's autocomplete is now always offered the most-uppercase spelling — `MCMASTER` over `McMaster`, `ELECTRONICS/POWER` over `Electronics/Power`, `RELAY` over `Relay`. That is stable, which is all DW-96 asked for, but it is also the spelling a human is least likely to have wanted, and no alternative was evaluated.
evidence: Asserted directly by the new tests as the intended outcome (`tests/integration/test_suggestion_order_collation.py` expects `VENDOR_UPPER`, `CATEGORY_UPPER`, `TAG_UPPER` in every case). DW-96 stated the defect as a plan-dependent survivor and asked only for a total ordering, so the change satisfies its contract; the spelling that wins fell out of choosing `utf8mb4_bin` rather than out of a product decision, and it is now pinned by tests and stated as contract in both public docstrings. The alternatives were never costed: the most FREQUENTLY stored spelling (a `GROUP BY LOWER(col) ORDER BY COUNT(*) DESC` key, which is what an operator's own usage implies), the most recently written one, or simply preferring a spelling that is not all-caps. Exposure is bounded and shrinking on the catalog side — `normalize_category_path` and `normalize_tag` canonicalize writes to lower case, so only rows predating those rules can differ by case — but the inventory fields (`vendor`, `location`, `sub_location`, `purchase_location`, `thread_size`) have no such canonicalization and accrue mixed casings from ordinary typing, which is where an operator will actually meet this. Closing it is a product decision first and a query change second. Related to [DW-96].
status: open
decision: 2026-07-28 Prefer the most frequently stored spelling — Change both `get_field_value_suggestions` implementations (app/mariadb_catalog_service.py, app/mariadb_inventory_service.py) to break the LOWER(column) tie by frequency -- a GROUP BY on the folded value ordered by COUNT(*) DESC -- keeping app/db.py's binary_order_key as the final tiebreak so the ordering stays total and deterministic. Update the integration expectations in tests/integration/test_suggestion_order_collation.py from the UPPER spellings to the most-stored ones, and restate the contract in both docstrings. This is what an operator's own usage implies and it degrades to today's answer when every casing appears once.

### DW-225: SQLite's `lower()` folds ASCII only, so the unit tier cannot represent MariaDB's case grouping for non-ASCII values
origin: spec-suggestion-order-collation-tiebreak-review-2
source_spec: `_bmad-output/implementation-artifacts/spec-suggestion-order-collation-tiebreak.md`
location: `app/mariadb_inventory_service.py::get_field_value_suggestions`, `app/mariadb_catalog_service.py::get_field_value_suggestions` (the `func.lower(column)` primary sort key), `tests/unit/` generally
severity: low
summary: The suggestion sort's primary key is `LOWER(column)`. Under MariaDB's `utf8mb4_unicode_ci` that ties case variants so the binary tiebreak decides between them; under SQLite, whose built-in `lower()` maps only `A-Z`, case variants carrying non-ASCII letters do NOT tie, the tiebreak never runs, and the two backends can offer different spellings of the same value.
evidence: Worked example on this checkout's code: stored `'aÉ'` and `'Aé'` are one Python dedup key (`str.lower()` folds both to `'aé'`), so exactly one is offered. MariaDB ties them on `LOWER()` and the collated tiebreak picks `'Aé'`; SQLite's `lower('aÉ')` returns `'aÉ'` unchanged, which sorts before `'aé'`, so it offers `'aÉ'`. Each backend is internally deterministic — DW-96's acceptance criterion is met either way — but a unit test that pins an offered spelling for non-ASCII case variants would be pinning SQLite semantics, not the deployed ones. Pre-existing: it is a property of SQLite's `lower()` and of the `LOWER()` key that predates this change, which only added a third sort key underneath. Not closed here because the spec's contract explicitly forbade registering a custom SQLite collation (that is DW-72's subject) and because the fix has to cover the bare `create_engine` sites `app/db.py`'s module docstring enumerates, not just the storage seam. Recorded so the divergence is written down where the suggestion ordering is reasoned about rather than only where collations are. Related to [DW-72], [DW-96].
status: open

### DW-226: `ErrorHandler` merges an arbitrary caller dict into a log record's `extra`, unredacted — the one live instance of the pattern this bundle just closed in the logging helpers
origin: spec-logging-redaction-completeness-review
source_spec: `_bmad-output/implementation-artifacts/spec-logging-redaction-completeness.md`
location: `app/error_handlers.py:74-81`, `app/exceptions.py:18-22`
severity: medium
summary: `current_app.logger.error(..., extra=log_data)` where `log_data['error_details']` is `WorkshopInventoryError.details` — a free-form caller dict — reaches the log with no `_redact_sensitive` pass, and unlike `log_operation`/`log_performance` this path has real callers and `error_details` IS in `JSONFormatter`'s emit allowlist, so whatever it carries is serialized verbatim.
evidence: Verified on this checkout. `WorkshopInventoryError.__init__(self, message, code=None, details: dict = None)` stores `self.details = details or {}`; `ErrorHandler` at `app/error_handlers.py:74-79` does `log_data.update({'error_code': error.code, 'error_details': error.details})` and line 81 passes `log_data` as `extra=`. `JSONFormatter` emits `error_details` (`app/logging_config.py:213-214`), so a `details` dict carrying a `csrf_token`/`password`/`authorization` key lands in structured output — which is exactly what DW-97 asked be made impossible, closed for the two helpers that have NO callers while this one was left open. Nothing leaks today: no current raise site puts request-derived data in `details` (`ValidationError` carries `field`/`value` as attributes, not in `details`). Two related hazards on the same line, which is why this is one entry: (1) the same `extra=` path raises `KeyError("Attempt to overwrite …")` from `logging.Logger.makeRecord` for any `details` key colliding with a `LogRecord` attribute — `name`, `module`, `filename`, `args` — taking down the error handler that was only meant to describe a failure; `_redact_merge` now guards the two sibling helpers against exactly that, and `log_data` is built with `error_id`/`user_id`/`request_url` and never audited for it. (2) `error.details` is also echoed to the client in `_handle_custom_error`'s response body (`app/error_handlers.py:93`), so the redaction question and the response-shaping question are the same question. Deliberately not fixed in this bundle: the spec's contract scoped the change to `app/logging_config.py` and its tests, and routing `error_handlers` through the choke point is a behaviour change to the error path that deserves its own decision — in particular whether the response body should be filtered by the same denylist. Related to [DW-97], [DW-99], [DW-43].
status: open

### DW-227: `operation`, `item_id` and `duration` are the three flat-merge keys left unprotected — a caller can forge them in the emitted JSON, or make the whole record vanish
origin: spec-logging-redaction-completeness-review-2
source_spec: `_bmad-output/implementation-artifacts/spec-logging-redaction-completeness.md`
location: `app/logging_config.py` (`_MODULE_RECORD_ATTRS` / `_UNMERGEABLE_KEYS`, `log_operation`), `app/logging_config.py::JSONFormatter.format`
severity: medium
summary: `_UNMERGEABLE_KEYS` nests every caller key that `makeRecord` refuses or that this module's filter/formatter owns, but deliberately excludes the helpers' own three fields — so a `details={'operation': …, 'item_id': …, 'duration': …}` still merges flat, and those three ARE in `JSONFormatter`'s emit allowlist, exactly like the `audit_data` this bundle did protect.
evidence: Reproduced on this checkout. `log_operation('add_item', item_id='JA1', duration_ms=5, details={'operation': 'FORGED', 'item_id': 'JA-EVIL', 'duration': 99999})` emits `{"message": "Operation 'add_item' on item JA1 completed in 5ms", "operation": "FORGED", "item_id": "JA-EVIL", "duration_ms": 99999}` — the structured fields contradict the message they were built beside. Worse, these three are also the last remaining route to the SILENT RECORD DROP that `_MODULE_RECORD_ATTRS` exists to close: they are the only caller-controlled keys that reach `json.dumps(log_entry, default=str)`, so `log_operation('add_item', details={'operation': <object whose __str__ raises>})` makes `format` raise, `logging` discards the record, and the emitted output is the empty string. That is precisely the `url` failure mode the bundle fixed one key at a time, still open on three. `details`/`context` are NOT in the allowlist, so no other merged key can do this. Nothing leaks today: neither sibling helper has a production caller. Deliberately not fixed in this pass: the spec's contract lists "do not 'fix' the pre-existing shadowing of `operation`/`duration`/`item_id` by caller keys — not this bundle" under **Never**, and adding them to `_UNMERGEABLE_KEYS` is exactly that change. Closing it means deciding whether the helpers' own fields win (nest the caller's copies, as with every other owned name) or the shadowing is intended, and saying so in one place instead of two. Related to [DW-97], [DW-226].
status: open

### DW-228: the log helpers' SCALAR parameters still run caller code outside every guard, so a logging call can still raise into its caller
origin: spec-logging-redaction-completeness-review-2
source_spec: `_bmad-output/implementation-artifacts/spec-logging-redaction-completeness.md`
location: `app/logging_config.py` (`log_operation` `item_id`/`duration_ms` gates and message, `log_performance` `end_time - start_time`, `log_audit_operation` `item_id`/`phase`/`operation_name` message parts)
severity: low
summary: `_has_payload` and `_message_field` were added so that no caller `__bool__`/`__len__`/`__contains__`/`__format__` runs outside a try — but only on the caller DICT parameters. The scalar parameters one line away are still gated with a bare `if item_id:` and rendered with a bare f-string, so an object passed as one of those propagates straight out of the logging call.
evidence: Reproduced on this checkout, all five raising: `log_operation('x', item_id=<__bool__ raises>)` → `RuntimeError`; `log_operation('x', item_id=<__format__ raises>)` → `RuntimeError`; `log_operation('x', duration_ms=<__bool__ raises>)` → `RuntimeError`; `log_audit_operation('x', 'success', item_id=<__bool__ raises>)` → `RuntimeError`; `log_performance('x', <__bool__ raises>, 1.0)` → `TypeError` from `end_time - start_time`, before any guard exists at all. The invariant the bundle states is unqualified ("A logging helper must never raise into its caller"), and `_has_payload`'s own docstring argues that "leaving it open on the gate would just move the asymmetry one line up" — which is what these are. This pass corrected both docstrings to say which surface is actually covered rather than closing the gap, because the parameters are single-purpose scalars (`str`, `int`, `float`) that no caller in the codebase passes an object to, and because `end_time - start_time` is a different problem from a gate: if the subtraction cannot be done there is no metric to log, so guarding it is a behaviour decision, not a wrapper. Closing it means routing the scalar gates through `_has_payload`, building every message part through `_message_field`-style guarded formatting, and deciding what `log_performance` should emit when the duration cannot be computed. Note the interaction with [DW-227]: guarding the GATE alone is not sufficient for `item_id`, since the unrenderable value then reaches the record and is dropped by the formatter instead. Related to [DW-97], [DW-227].
status: open
...
### DW-247: `update_product` writes an audit record with `item_after` only, so the quantity history Story 5.1 put in `to_dict()` cannot actually be read back
origin: spec-5-1-tri-state-quantity-and-location-followup-review
source_spec: `_bmad-output/implementation-artifacts/5-1-tri-state-quantity-and-location.md`
location: `app/mariadb_catalog_service.py:506` (`update_product`), `:452` (`create_product`), `app/database.py:342-347`
severity: low
summary: Every `update_product` audit record carries `item_after=audit_snapshot` and no `item_before`, so the log states the resulting values and never the ones they replaced. Story 5.1 added `quantity_on_hand` / `quantity_verified_at` to `Product.to_dict()` and justified it in a comment with "the quantity is precisely the value whose history an operator would want to read" — but a sequence of after-snapshots is not a history: 12 → 3 → 0 over six months is indistinguishable from a typo correction unless a reader correlates adjacent records by timestamp and assumes no interleaved edits to other fields.
evidence: Pre-existing, and verified as such: `git show 4278217:app/mariadb_catalog_service.py | grep -n 'item_before\|item_after'` returns six `item_after=` call sites and zero `item_before=` at the story's baseline, so this story neither created nor changed the shape. It is filed now because this story is what made the omission matter — before it, the product columns were all values a reader could recover by looking at the row, and a count is the first one where "what did it used to be" is the question the log exists to answer. Closing it means capturing `product.to_dict()` before the field loop in `update_product` and passing it as `item_before`, which the audit helper already accepts (`create_product` legitimately has no before-state and should stay as it is). Worth checking the other four `item_after`-only call sites in the same file at the same time.
status: open

### DW-248: The merged location autocomplete does two unbounded, unindexed, fully-sorted table scans per keystroke
origin: spec-5-1-tri-state-quantity-and-location-followup-review
source_spec: `_bmad-output/implementation-artifacts/5-1-tri-state-quantity-and-location.md`
location: `app/main/routes.py:4259` (`inventory_field_suggestions`), `app/mariadb_catalog_service.py:1142` (`get_product_location_suggestions`), `app/mariadb_inventory_service.py:782+`, `migrations/versions/2c837402a89a_add_product_stock_and_location.py`
severity: low
summary: Neither `inventory_items.location`/`sub_location` nor the `products.location`/`sub_location` this story added carries an index, and both suggestion queries are deliberately unbounded (the item-side comment states the trade: "a full ordered read per lookup"). Story 5.1 made the `location`/`sub_location` endpoint query BOTH services, so the debounced autocomplete now costs two full `ORDER BY LOWER(col), col COLLATE utf8mb4_bin` filesorts over unindexed VARCHARs per keystroke instead of one.
evidence: `grep -n 'Index(' app/database.py` returns exactly one hit in the whole module (`ix_item_photo_associations_ja_id_order`), so neither table's location columns are indexed; the new migration issues four bare `op.add_column` calls and no `op.create_index`. The doubling is real but the base cost is pre-existing and deliberate — the item side chose unbounded reads knowingly — which is why this is deferred rather than patched, and why the story's `Block If` clause ("a migration proves to need anything beyond `op.add_column`") would have made adding an index a spec-level decision rather than an implementation one. Impact scales with catalog size and is nil at present volumes; it is filed so the index lands with the first catalog large enough to feel it. Closing it means a prefix index on all four columns in a follow-up migration, and re-measuring whether the unbounded read is still the right trade once one exists.
status: open

### DW-249: Duplicate-product prefill carries none of the stock fields, and a reorder threshold is the one where copying is arguably right
origin: 5-2-reorder-threshold-and-derived-effective-low-review
source_spec: `_bmad-output/implementation-artifacts/5-2-reorder-threshold-and-derived-effective-low.md`
location: `app/main/routes.py:1299` (`_PRODUCT_PREFILL_ARGS`), `:1240` (`_product_form_data`)
severity: low
summary: The "duplicate this product" prefill copies the descriptive fields and deliberately none of the Story 5.1/5.2 stock fields, so a duplicated product starts with no quantity, no location and no reorder threshold. That is obviously right for a count and a shelf — they are facts about the physical unit in front of the operator — but a reorder threshold is a per-product *policy*, and the product most likely to be duplicated (a second value of the same part family) is the one most likely to want the same policy.
evidence: Pre-existing shape, extended rather than created by this story: `git show fac9137:app/main/routes.py` shows `_PRODUCT_PREFILL_ARGS` already omitting `quantity_on_hand`, `location` and `sub_location` at the baseline, and Story 5.2 simply did not add its field either. Deferred rather than patched because it is a product decision, not a defect: the story's Boundaries never mention the duplicate flow, so there is no captured intent to implement against, and either answer is defensible. Closing it means deciding whether `reorder_threshold` (and only it, of the four stock fields) joins the prefill, and if so saying so in the user manual's duplicate section — an operator who does not notice a copied threshold gets a low-stock signal on a product they never set a policy for.
status: open

### DW-250: The Effective-Low expression is advertised to two future query consumers, and nothing it filters on is indexable as written
origin: 5-2-reorder-threshold-and-derived-effective-low-followup-review
source_spec: `_bmad-output/implementation-artifacts/5-2-reorder-threshold-and-derived-effective-low.md`
location: `app/database.py` (`Product.is_effective_low`'s `@expression`), `migrations/versions/d4f18b0a6c37_add_product_reorder_threshold.py`, `app/database.py` (`Product.__table_args__`)
severity: low
summary: `Product.is_effective_low`'s docstring hands its SQL half to "Story 5.6's reorder view and Epic 8's stock facet" as the way to "filter in the DATABASE instead of loading the catalog and sieving it in Python" — but the predicate is an INTER-COLUMN comparison (`quantity_on_hand <= reorder_threshold`) behind two `IS NOT NULL` guards, and `~Product.is_effective_low` wraps the whole thing in a `NOT`. Neither shape is usable by an ordinary B-tree index on either column, and neither column carries one anyway, so both stated consumers will full-scan `products`.
evidence: `grep -n 'Index(' app/database.py` returns exactly one hit in the module (`ix_item_photo_associations_ja_id_order`); `Product.__table_args__` declares none for the stock columns, and the new migration issues one bare `op.add_column` and no `op.create_index`. Scoping the index out of THIS story was correct and is not the finding — the story's `Block If` makes the migration needing "an index" a spec-level decision, and this story adds no query at all (the only read surface is one detached instance on the product detail page), so there is nothing here to be slow. It is filed because the omission is invisible: the property's own docstring is what creates the expectation of a database-side filter, and no record exists that the expectation is currently unbacked. Precedent for the shape of the trade is DW-248, filed against the same table's unindexed `location` columns. Closing it is not a plain `op.create_index`, which is why it needs its own thought: an inter-column comparison cannot use one. The realistic options are a MariaDB generated/virtual column carrying the predicate with an index on it (which reintroduces a stored derived value, and AD-6 exists to forbid exactly that — so it needs an explicit decision, not a quiet migration), a covering index on `(reorder_threshold, quantity_on_hand)` that at least turns the scan into an index-only one, or accepting the scan and saying so in the docstring where the promise is currently made. Worth settling when Story 5.6 lands the first real query, and re-checked after Story 5.3 widens the predicate with `stock_status`, which is indexable on its own and changes the arithmetic.
answer (story 5-3-manual-stock-status): The predicate HAS now widened — `Product.is_effective_low` is `or_(stock_status IN ('low','out'), and_(<the three-part threshold branch>))` — and the recorded decision is **no index in this story**, deliberately rather than by omission. Three reasons, in the order they matter. (1) There is still no query consumer: Story 5.3 adds no `filter(Product.is_effective_low)` anywhere in `app/`, and its only read surface is the same single detached instance on the product detail page, so there is nothing here to be slow. Story 5.6 lands the first list, which is the point at which a real query plan can be measured rather than guessed at. (2) `stock_status` is a four-value column with a `NOT NULL DEFAULT 'unknown'`, so in any realistic catalog the overwhelming majority of rows share one value — a B-tree on it would be low-cardinality, and the selective side (`IN ('low','out')`) is the small minority the optimizer would have to be persuaded to use it for. (3) It would not remove the scan in any case: the OR's other arm is still the inter-column comparison this entry was filed about, so `stock_status IN (…) OR (…)` cannot be satisfied from an index on `stock_status` alone — the threshold branch has to be evaluated for every row that fails the status test, which is a full scan of the table minus the flagged minority. The realistic options this entry already lists are unchanged by the widening; the generated/virtual-column option remains forbidden by AD-6, which the migration `3130eeaa8ef6` states in its own docstring. Left OPEN for Story 5.6 to settle with a measurement.
status: open

### DW-251: The two encodings of Effective Low provably disagree for a trailing-space `stock_status`, and the disagreement is pinned as a passing test rather than filed as work
origin: 5-3-manual-stock-status-followup-review
source_spec: `_bmad-output/implementation-artifacts/5-3-manual-stock-status.md`
location: `tests/integration/test_effective_low_predicate.py:248-277` (`test_a_trailing_space_is_the_folded_spelling_the_pin_does_NOT_close`), `app/database.py` (`Product.is_effective_low`, the `stock_status` column comment), `migrations/versions/3130eeaa8ef6_add_product_stock_status.py`, `tests/integration/conftest.py`
severity: low
summary: `products.stock_status` is pinned to `utf8mb4_bin`, which closes case and accent folding but is a PAD SPACE collation on MariaDB — so for a stored `'low '` the SQL half of `Product.is_effective_low` answers TRUE while the Python getter's `LOW_STOCK_STATUS_VALUES` membership answers False. AD-6's requirement that the two bodies "agree row for row" therefore has a known, reproducible exception, and the story turned it into an asserted expectation (`assert python_low is False; assert sql_low is True`) rather than a ledger item.
evidence: Asserted directly by the named integration test against a real MariaDB testcontainer, seeded through a raw UPDATE that bypasses the write path. The disagreement is real but currently unreachable in service: `_apply_stock_status_assertion` is the pair's only writer on both the create and the update path and refuses any string outside `StockStatus` — including `' low '` and `'   '`, which the route tier refuses too — so only a hand-run UPDATE or a restored backup can produce the spelling. Not patched in this story for the reason the test docstring gives: `utf8mb4_nopad_bin` would close it at the cost of making `stock_status` the schema's only NO PAD column and splitting the single `utf8mb4_bin` pin that `products.internal_id` (DW-73/`a977ca7315df`) and every schema guard share. Filed because the decision currently lives only in a test docstring: the ledger is where a deliberately-unclosed AD-6 exception belongs, and Story 5.6 is the first consumer of the SQL half — a reorder list is precisely where a row that reads low in the database and not-low on its own detail page becomes visible to an operator. Closing it means choosing between `utf8mb4_nopad_bin` on this column, an `rtrim()`/`coalesce`-style normalization inside the `@expression` (which adds a per-row function to the very predicate DW-250 is about the indexability of), and accepting the boundary explicitly in `app/database.py`'s docstring where the row-for-row promise is made. Whichever is chosen, the test docstring names the three other places that must move with it.
status: open

### DW-252: The product edit form is last-write-wins, and the stock-status `<select>` makes a stale page's re-post silently withdraw an assertion made after it rendered
origin: 5-3-manual-stock-status-followup-review
source_spec: `_bmad-output/implementation-artifacts/5-3-manual-stock-status.md`
location: `app/main/routes.py` (`product_edit`'s present-keys loop and its `_render_data()` merge), `app/mariadb_catalog_service.py` (`update_product`, `_apply_stock_status_assertion`)
severity: low
summary: `product_edit` has no optimistic-concurrency check: the POST carries only values, never the baseline they were read from, so whichever page is submitted last wins every field it posts. With two edit pages open on one product, saving `stock_status=low` on the newer one and then re-posting the older one — which still carries the `unknown` it rendered with — clears the flag and NULLs `stock_status_at`, and the older page never showed the operator that a flag existed.
evidence: Reproduced by the edge-case review against the real routes: two GETs of `/products/edit/<id>`, a save of `low` through the second, then a verbatim re-post of the first, leaves `stock_status='unknown'` with `stock_status_at IS NULL`. Filed as pre-existing rather than caused by this story: the same clobber applies to every field on the form (a stale `description`, `quantity_on_hand` or `reorder_threshold` overwrites a newer one identically, and for the counts it additionally re-stamps `quantity_verified_at`, since the changed-value rule sees the stale value as a change), and `git show 8d77070:app/main/routes.py` shows the same value-only POST shape before this story. What Story 5.3 changes is the visibility, not the mechanism: a `<select>` posts its key on every save and its default is the withdrawal state, so unlike a text input there is no blank-means-untouched spelling of "I did not look at this field", which is the same property the story's own Design Notes call out as the reason the stamp rule is changed-value rather than key-present. Not patched because closing it is a design decision well outside this story's Boundaries — the realistic options are an `updated_at` round-trip rejected on mismatch (which needs an operator-facing conflict page and a decision about what the loser's edits do), a per-field `*_seen` baseline echoed through the form, or accepting last-write-wins and saying so in `docs/user-manual.md`'s editing section. Worth settling before any multi-operator use; for the single-operator workshop this app is built for, the exposure is two tabs on one bench.
status: open

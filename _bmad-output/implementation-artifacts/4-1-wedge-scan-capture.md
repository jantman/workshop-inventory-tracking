...
**Follow-up review recommendation: true.** This pass introduced new client state (`mergedResidue`, `inFlightText`) and changed what an `Enter` arriving during a request *means* — a semantic that now has three distinct outcomes where it previously had one — and moved the escaping boundary into `notify()`, which is a security-relevant relocation. None of that has been independently reviewed. The decisive evidence, though, is the pattern: each of the three passes has found that the *previous* pass's fix was incomplete on the same double-scan path. Pass 2 found pass 1's `refocus()` guard defeated one line later by an unguarded `select()`; pass 3 found pass 2's residue *selection* closed only the keystroke path and left a bare `Enter` posting the concatenation, and separately that the in-flight guard misread a CRLF terminator as a dropped scan. Three for three is not a run of bad luck; it is a signal that this specific interaction is harder than it looks and has not yet converged.
...
- `app/static/js/scan-capture.js` — new; the `ScanCapture` object literal: Enter-to-POST, an in-flight guard that tells a duplicate CR+LF terminator from a genuine second burst, refusal of contaminated (`SCAN1SCAN2`) text, an abort timeout, conditional clear/restore of the field, conditional refocus, an IME guard, and a single escaping boundary in `notify()`.
...
- `tests/unit/test_scan_routes.py` — new; 52 tests covering every server row of the I/O matrix, the trim rule in isolation including its exact boundary, byte-exact control-character preservation, the no-service contract, and CSRF exemption proven behaviorally against an app with protection enabled.
- `tests/e2e/test_wedge_scan.py` — new; 33 tests covering field presence across five pages (including the 404 page), verbatim posting, clear-and-refocus, the failure paths, the late-response races, the double-scan and CRLF-terminator paths, an end-to-end comparison of the client and server trim rules, and the NFR9 JA-ID regression guard.
- `docs/images/screenshots/` — 12 PNGs plus `metadata.json` regenerated for the navbar change.

**Review findings breakdown.** Three review passes, each with 2 reviewers (adversarial + edge-case).
...
- *Second follow-up pass:* **0 intent_gap, 0 bad_spec, 10 patch, 2 defer, 18 reject**. Both mediums were on the double-scan path again. A wedge with a CR+LF suffix sends *two* `Enter` presses for one scan, and the guard counted the second as a dropped scan — putting a "rescan this item" warning on every scan and inviting the double-processing that 4.3/4.5 make expensive; the guard now recognises a duplicate terminator by the field being unchanged. And the previous pass's residue *selection* closed only the path where the next input is a keystroke: a bare `Enter` on `SCAN1SCAN2` still posted the concatenation as a valid scan, so contaminated text is now flagged and refused outright. The eight lows: an abort during the body read losing its "may have been received" warning, escaping moved into `notify()` as a single enforceable boundary, an IME-composition guard, a duplicated tooltip, and four test-strength gaps (the unasserted no-service contract, a CSRF test that proved nothing behaviorally, no end-to-end comparison of the two trim-rule copies, and "every page" asserted from 4 of 19 templates).

No spec-repair loopback was required in any pass (`review_loop_iteration` stayed 0). The 10 deferrals are in `deferred-work.md`: no app-wide request-body limit, the unescaped `showToast` sink in `main.js`, navbar inputs hidden below the `lg` breakpoint, `inventory-add.js`'s document-level scan-mode handler swallowing keystrokes, whether a wedge can physically type ASCII control characters into a text input (a real risk to Story 4.4), the screenshot generator writing a truncated manifest, the trim rule living as a private symbol in `routes.py` that Story 4.2 must import or duplicate, the transport's at-least-once semantics on timeout once 4.3/4.5 give the endpoint side effects, `main.js`'s global shortcut handler processing a wedge burst whenever no field is focused (a `/` in a GS1 Digital Link payload fires Focus Search; `?` opens the help modal), and the CSRF exemption needing a decision once the endpoint acquires side effects. Rejections across both passes were design decisions the intent contract states explicitly (`outcome: 'unrouted'` not routing anything; no success toast; no timeout terminator; focus-required capture with no document-level handler), unevidenced layout and browser-compatibility speculation, one artifact of the review input rather than the code, and suggestions that would have made things worse — notably a client-side `maxlength="4096"`, which would silently truncate an over-long payload into a *valid* 200 instead of the loud rejection the server gives today.

**Verification performed.**
- `nox -s tests` — green: **1241 passed**, 345 deselected (+3 from this pass's new unit tests).
- `nox -s e2e` — green: **354 passed, 1 skipped** in 19m55s, including all 38 `test_wedge_scan.py` cases (28 before this pass, +10 from the new guards and parametrize cases) and the untouched `test_ja_id_lookup.py`.
...
- `nox -s screenshots_verify` — green: all 12 screenshots valid PNG, all under 500 KB, 1.70 MB total. This pass's only template change removes a redundant `title` attribute from an already-`aria-hidden` icon, which has no visual effect, so the PNGs were not regenerated again.
...
- **The double-scan handling is correct by construction but unexercised against a real scanner's burst timing.** The in-flight guard now distinguishes a duplicate CR+LF terminator from a second burst, warns on the latter, and refuses to submit the concatenation a dropped burst leaves behind. The CRLF case is exercised with real key presses; the burst cases are still synthetic Chromium events, not a Tera HW0009 emitting two labels back to back. Whether the deployed scanner actually carries a CR+LF suffix is unknown — the fix is correct either way, but the case that motivated it has not been observed on the hardware.
- **Which `Enter` means what is now inferred from the field's contents.** A duplicate terminator is recognised by the value being unchanged, and a second burst by the value having grown. That is the only signal available to a `keydown` handler, and it is right for every case reasoned through here, but it is an inference rather than a fact the browser reports.

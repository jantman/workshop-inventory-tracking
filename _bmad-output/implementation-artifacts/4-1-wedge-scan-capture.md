...
- reject: 18
- addressed_findings:
  - `[medium]` `[patch]` One scan can send TWO `Enter` presses — HID has no separate LF key, so a wedge programmed with a CR+LF suffix emits Return twice in the same burst and the second always lands inside the first one's in-flight window. The previous pass's guard treated every such Enter as a dropped scan, so a CRLF-suffixed scanner would have put a "rescan this item" warning on *every single scan* and invited the operator to double-process an item that was captured — the exact double-apply the deferred at-least-once entry warns about, arriving by a different route once 4.3/4.5 add side effects. The guard now distinguishes a duplicate terminator (field value identical to what is in flight) from a genuine second burst (field has grown), and is silent for the former. Covered by `test_crlf_terminator_posts_once_and_is_not_called_a_dropped_scan`, which uses **real key presses** and stubs only `fetch` — the previous pass's synthetic-`KeyboardEvent` test could not have distinguished the two cases because it never modelled a terminator at all.
  - `[medium]` `[patch]` The previous pass fixed "a rescan concatenates and POSTs `SCAN1SCAN2`" by *selecting* the residue so the next burst would overwrite it. That closes only the path where the next input is a keystroke. A bare `Enter` — the obvious response to the warning the guard had just shown, and what a repeat-trigger scanner emits — still submitted the concatenation as one valid 200 scan, which is the silently *wrong* scan the fix existed to prevent. Contaminated text is now flagged (`mergedResidue`) and refused outright: the field is cleared, a danger toast explains why, and nothing is posted. Matched by `startsWith` rather than equality so the case where focus had left the field (and `selectIfActive` could not select) is covered too. Covered by `test_bare_enter_on_merged_residue_is_refused_not_posted`.
  - `[low]` `[patch]` An abort that fired while the response *body* was being read surfaced as a null `data` rather than a rejected `fetch`, so it was reported as a generic failure and lost the "the server may or may not have received it" warning that the same abort gets on the network path. Now keyed off `controller.signal.aborted` on both paths.
  - `[low]` `[patch]` Escaping was applied at each call site, with nothing at the call site distinguishing an already-escaped string from a raw one — which is precisely how Stories 4.2/4.3 would reintroduce the injection the previous pass closed. `notify()` is now the single escaping boundary and every caller passes plain text.
  - `[low]` `[patch]` An IME commit fires `Enter` as well; acting on it posted a partial composition. Added an `isComposing`/keyCode-229 guard. Covered by `test_ime_composition_enter_does_not_submit`.
  - `[low]` `[patch]` The icon `<span>` and the input carried two different `title` tooltips on one control, so hover text changed depending on which half of the control the pointer was over. Removed the redundant one; the icon is already `aria-hidden`.
  - `[low]` `[patch]` `tests/unit/test_scan_routes.py`'s module docstring stated "the endpoint touches no database and constructs no service — that is itself part of the contract" and then asserted it nowhere; wiring `_get_catalog_service()` in would have kept all 49 tests green. Added `test_endpoint_constructs_no_catalog_service`.
  - `[low]` `[patch]` `test_endpoint_is_csrf_exempt` reads Flask-WTF's private `_exempt_views` registry, and `TestConfig` sets `WTF_CSRF_ENABLED = False`, so nothing behavioral was proven — the `client` fixture would accept a tokenless POST with or without the decorator. Added `test_csrf_exemption_holds_with_protection_actually_enabled`, which builds an app with protection genuinely on and proves the protection is real via a control route before trusting the 200.
  - `[low]` `[patch]` Nothing compared the two copies of the trim rule end to end: the client never reads `data.raw`, so `_SCAN_TRIM` and `ScanCapture.stripOuter` were each only ever pinned against their own expectations. Added `test_server_echo_matches_the_clients_own_trim_rule`, which drives the real field and asserts the server's echo equals the client's own function across the boundary set (NBSP, BOM, VT/FF) where JS `trim()` and `str.strip()` each disagree with the rule.
  - `[low]` `[patch]` "Present on every page" was asserted from 4 of 19 templates and omitted the error pages — the pages an operator lands on without choosing to, and the ones a route-driven check misses. Added the 404 page to the parametrize (`errors/404.html` does extend `base.html`). Also pinned that outer whitespace counts toward `MAX_SCAN_LENGTH`, since the check's position relative to `_clean_scan_input` was otherwise invisible.

### 2026-07-25 — Review pass (follow-up 3)
- intent_gap: 0
- bad_spec: 0
...
- reject: 23
- addressed_findings:
  - `[medium]` `[patch]` The concatenation guard covered the two paths the previous passes came in through and missed the one the operator actually stares at: the **failure-retry** text. `handleFailure` restored the failed scan and explicitly set `mergedResidue = null`, leaving the *selection* as the only thing stopping the next burst from appending — and a single click destroys it. Its other branch (a burst already typing on top of the failed text) marked nothing at all, so the Enter that burst was about to send POSTed `FIRSTSECOND` as one valid 200 scan. Rather than add a third special case, the rule is now general: the client remembers what it itself put in or submitted from the field (`retainedText`, `mergedResidue`) and `isContaminated()` refuses any value that *extends* it, while still allowing a value that *equals* restored text — that is a legitimate retry. Fixing it also exposed that `flagMergedResidue`'s `select()` is only safe from the keydown path, where the burst has finished: called from an async response handler it selects mid-burst, so the burst's remaining keystrokes replace the field and leave a *truncated* payload that no longer matches the residue and would submit. The async paths now use `markMergedResidue()`, which records without selecting. Covered by `test_restored_text_does_not_concatenate_onto_the_next_burst` (which loses the selection by clicking away, the way an operator does) and `test_failure_landing_mid_burst_refuses_the_concatenation`.
  - `[low]` `[patch]` A successful scan could produce **zero** feedback. `handleSuccess`'s declined-clear branch returned silently, and the story's own reason for making the failure toast mandatory — retained text is indistinguishable from a scan that never fired — applies verbatim to a field that could not be cleared. Worse, the refusal the operator's next Enter earns talks about the *field*, not about the item already captured, so the natural reading is "nothing was sent" and the natural response is to rescan an item the server has taken. The branch now says the scan was accepted. Covered by `test_accepted_scan_says_so_when_the_field_cannot_be_cleared`.
  - `[low]` `[patch]` The CR+LF suffix that pass 3 taught the guard to recognise on the *captured* path still double-warned on the *dropped* path: the second Return was compared only against the text in flight, so it took the "field has GROWN" branch again and one dropped item produced two identical "rescan this item" toasts. Now compared against the value already announced (`warnedText`). Covered by `test_one_dropped_burst_warns_once_even_with_a_crlf_suffix`.
  - `[low]` `[patch]` Refusing merged text *erased* it, which is the same loss pass 2 fixed on the failure path by putting unrestorable text into the toast. The discarded text now goes into the toast too.
  - `[low]` `[patch]` `notify()` could throw: `showToast` ends in `new bootstrap.Toast(...)` and Bootstrap is a CDN script, so with the CDN unreachable the exception propagated out of the in-flight guard *before* `flagMergedResidue()` ran, leaving the concatenation submittable. Scan-state mutation now precedes every `notify()`, and `notify()` swallows its own failure to `console.error`.
  - `[low]` `[patch]` The trailing `.catch()` — a bug thrown inside a handler — logged to the console and told the operator nothing, which is exactly the silence the mandatory failure toast exists to prevent. It now reports an unknown outcome without touching the field.
  - `[low]` `[patch]` `refocus()`'s allowlist (`input`, `body`, `null`) missed two "nothing is focused" states a browser really reports — `documentElement`, and a *detached* node, which is what a dismissed toast's own close button becomes — so a successful scan cleared the field and left it unfocused, dropping the next burst into the already-deferred global-shortcut path.
  - `[low]` `[patch]` `errorMessage` trusted the server's `error.message` for both type and length: a non-string rendered as `[object Object]` and an unbounded one pushed the toast off screen. Now type-checked and truncated — the same distrust of server-supplied strings that pass 2's escaping fix established.
  - `[low]` `[patch]` The unrestorable-scan toast told the operator to read the text off while control characters render as *nothing* in the DOM — so an ISO/IEC 15434 envelope surfaced as fragments and a false claim. Added `printable()`, the client-side counterpart of the server's `repr` logging, used for both the unrestored and the discarded text.
  - `[low]` `[patch]` The endpoint's diagnostics were inconsistent across exactly the two paths a wedge investigation uses: "blank after trimming" logged no bytes at all and the over-length rejection logged only a count — "a scanner is emitting only its suffix" and "a scanner is emitting a runaway payload" being the two likeliest real faults. Both now log `repr` of what arrived.
  - `[low]` `[patch]` The capture log echoed the payload at whatever length was sent, and `repr` of a control-character-heavy payload is several times longer than the payload — an amplification any client can drive at an unauthenticated, CSRF-exempt, unthrottled endpoint. Bounded by `_SCAN_LOG_CHARS = 512`, with the true length still logged so a truncated line is recognisable. Covered by `test_logged_scan_is_bounded`; the *rate* is deferred.
  - `[low]` `[patch]` `tests/unit/test_scan_routes.py`'s module docstring claimed "no storage fixture is needed anywhere in this module — that is itself part of the contract" while `test_csrf_exemption_holds_with_protection_actually_enabled` (added last pass) takes `test_storage`. The same class of unasserted claim pass 3 fixed by adding a test; here the claim was simply false.
  - `[low]` `[patch]` The logging pass 1 added was pinned by nothing — no test asserted that a captured scan is logged, at what level, or that the rejection paths log at all, so the one endpoint whose purpose is "did the scan arrive" could lose its only server-side record silently. Added `TestScanLogging` (4 cases).
  - `[low]` `[patch]` Three failure-path e2e tests asserted only the toast's CSS *class*, so showing the merged-residue message where the failure message belongs would have passed — as would any unrelated danger toast the page raised. They now assert the message.
...
**Follow-up review recommendation: true.** This pass replaced the accumulated special cases with a general invariant — the client now refuses any field value that *extends* text it put there or submitted itself — and that restructuring (`retainedText`, `isContaminated()`, and the new `markMergedResidue()`/`flagMergedResidue()` split for "is it safe to select mid-burst") has not been independently reviewed. It also added a toast to a branch that was previously silent, changing what a successful scan looks like to the operator, and it changed what reaches the server log on all three paths. Fourteen patches is significant by volume alone. The honest note for whoever reads this next: four consecutive passes have each found a hole in the same double-scan interaction, but the previous three added a case and this one removed the case analysis, so a fifth pass finding only lows would be the first real evidence of convergence.
...
**Files changed.**
- `app/main/routes.py` — new `POST /api/scan` (`api_scan`, `@csrf.exempt`), `MAX_SCAN_LENGTH = 4096`, `_SCAN_LOG_CHARS = 512`, and `_clean_scan_input()` implementing the narrow trim rule; all rejections go through the AD-13 object-error envelope, and every path logs `repr` of what arrived, bounded.
- `app/static/js/scan-capture.js` — new; the `ScanCapture` object literal: Enter-to-POST, an in-flight guard that tells a duplicate CR+LF terminator from a genuine second burst and warns once per dropped burst, a single contamination rule that refuses any value extending text the client itself placed or submitted, an abort timeout, conditional clear/restore/refocus, an IME guard, control-character-visible operator messages, and a single escaping boundary in `notify()` that cannot throw out into scan-state logic.
- `app/templates/base.html` — the `#scan-input` navbar block (deliberately `type="text"`, no `name` containing `search`, so `main.js`'s `/` shortcut cannot hijack it) plus the global `<script>` tag. Unchanged by this pass.
- `tests/unit/test_scan_routes.py` — new; 57 tests covering every server row of the I/O matrix, the trim rule in isolation including its exact boundary, byte-exact control-character preservation, the no-service contract, CSRF exemption proven behaviorally against an app with protection enabled, and the diagnostic log on the capture and all three rejection paths.
- `tests/e2e/test_wedge_scan.py` — new; 42 tests covering field presence across five pages (including the 404 page), verbatim posting, clear-and-refocus, the failure paths, the late-response races, every double-scan and CR+LF-terminator path, an end-to-end comparison of the client and server trim rules, and the NFR9 JA-ID regression guard.
- `docs/images/screenshots/` — 12 PNGs plus `metadata.json` regenerated for the navbar change (in an earlier pass; the navbar is untouched since).

**Review findings breakdown.** Four review passes, each with 2 reviewers (adversarial + edge-case).
...
- *Follow-up pass:* **0 intent_gap, 0 bad_spec, 12 patch, 2 defer, 17 reject**. Three mediums: `refocus()` defeated one line later by an unguarded `select()`; the in-flight guard dropping the second `Enter` but not the second burst's *characters*, so a rescan POSTed a silently **wrong** scan; and the "operator already rescanned" branch discarding the failed text while the toast claimed it was kept.
- *Second follow-up pass:* **0 intent_gap, 0 bad_spec, 10 patch, 2 defer, 18 reject**. Both mediums on the double-scan path: a CR+LF wedge's second Return counted as a dropped scan, and a bare `Enter` still POSTing the `SCAN1SCAN2` concatenation the previous fix only *selected*.
- *Third follow-up pass (this one):* **0 intent_gap, 0 bad_spec, 14 patch, 2 defer, 23 reject**. The one medium was the concatenation guard's remaining blind spot — the failure-retry text, where the restore explicitly cleared the residue flag and left the selection as the only protection, and where a failure landing mid-burst marked nothing at all. Fixing it generalized the rule and exposed that selecting a residue from an asynchronous handler truncates an in-progress burst. The thirteen lows: an accepted scan with no signal at all on the branch that cannot clear the field, a doubled warning for one dropped burst under a CR+LF suffix, refused text erased instead of surfaced, `notify()` able to throw out of the in-flight guard and skip the flag (Bootstrap is a CDN script), a handler-bug path that told the operator nothing, `refocus()` missing two real "nothing is focused" states, a server `error.message` trusted for type and length, control characters invisible in a toast claiming the text is readable, two server rejection paths logging no bytes, an unbounded log echo on an unthrottled endpoint, a false module docstring, logging pinned by no test, and three failure-path e2e tests asserting only a CSS class.

No spec-repair loopback was required in any pass (`review_loop_iteration` stayed 0). Twelve deferrals now sit in `deferred-work.md`; this pass added two: no rate limiting anywhere in the app, which matters more for a CSRF-exempt endpoint that logs per request (the per-request cost was bounded here, the rate was not), and `main.js`'s global shortcut handler skipping its own in-input early-return whenever Ctrl/Meta/Alt is held — so a wedge transmitting control characters as Ctrl chords can open the help modal with the scan field properly focused, which is distinct from the already-deferred unfocused case and unfixable inside this story's NFR9 boundary. This pass's rejections were: design decisions the intent contract states explicitly (Enter-only termination, so a Tab-suffixed or Enter-embedded payload is out of scope; the I/O matrix's deliberate collapse of every malformed body into `invalid_field` on `raw`), two findings already covered by existing ledger entries (`MAX_CONTENT_LENGTH`, the navbar collapse breakpoint), claims that do not reproduce (an identical payload rescanned in flight cannot leave the field equal to the in-flight text, because the second burst's characters append), speculative operator behavior whose proposed guard would have reopened the hole it closed (clearing the residue flag on `input`), cosmetic template inconsistencies whose fix would restale twelve screenshots for no behavior change, a non-issue from the function-scoped `page` fixture, and an artifact of the review input (screenshots absent from a diff scoped to `app` and `tests`).

**Verification performed.**
- `nox -s tests` — green: **1246 passed**, 359 deselected (+5 from this pass's logging tests).
- `nox -s e2e` — green: **358 passed, 1 skipped** in 19m58s, with no reruns, including all 42 `test_wedge_scan.py` cases (+4 this pass) and the untouched `test_ja_id_lookup.py`.
- `git diff 80d5212 -- app/static/js/main.js app/static/js/inventory-move.js app/static/js/inventory-shorten.js` — empty. NFR9's no-touch boundary holds.
- `node --check app/static/js/scan-capture.js` — clean.
- Screenshots not regenerated: `app/templates/base.html` is byte-identical to the previous pass, so the committed PNGs still match the shipped navbar.

**Residual risks.**
- **Whether a real wedge can deliver ISO/IEC 15434 control characters at all.** The server preserves `\x1d`/`\x1e`/`\x04` byte-for-byte and is unit-tested for it, but browsers do not put non-printable characters into a text input from keystrokes — and `<input type="text">` value sanitization additionally strips CR and LF, so three of the four characters in the trim rule are unreachable from the intended client. The rule is still correct for any client; the risk is that Story 4.4 relies on a preservation path a physical scanner cannot exercise. Settling it needs the hardware.
- **Which `Enter` means what, and which text is contaminated, are both inferred from the field's contents.** A duplicate terminator is recognised by the value being unchanged, a second burst by its having grown, and a concatenation by its extending text the client itself placed there. That is the only signal a `keydown` handler has, and it is right for every case reasoned through here, but it remains an inference. An operator who hand-edits the *tail* of refused text can defeat the prefix match.
- **The double-scan handling is correct by construction but unexercised against a real scanner's burst timing.** The CR+LF cases use real key presses; the mid-burst race cases are synthetic Chromium events, not a Tera HW0009 emitting two labels back to back. Whether the deployed scanner carries a CR+LF suffix is still unknown.
- **The field is live on every page and does nothing yet.** By design — routing arrives in 4.2/4.3/4.5 — but until then an operator scanning a catalog part gets a cleared field and no outcome.
- **Two navbar scan inputs coexist.** Deliberate and argued in Design Notes; the operator has to know which field a barcode belongs in. Reconciling them is a post-4.5 decision.
- **The trim rule now exists in three places** — `_SCAN_TRIM`, its JavaScript mirror `ScanCapture.stripOuter`, and the tests that pin each. `test_server_echo_matches_the_clients_own_trim_rule` compares the Python and JS results end to end, but only over the boundary set it enumerates. Deferred; the rule should move to a pure util before Story 4.2 adds a fourth consumer.
